---
title: "Exploratory Data Analysis for Continental Bioregion"
...
# large_net_threshold <- 100  # Threshold for "large" networks in plots
...
#   theme_minimal()
 ...
 #     labs(
 #         title = "Distribution of Pollinator Occurrence Frequency",
 #         x = "Frequency (proportion of networks)",
...
#   theme_minimal()
 ...
 #     labs(
 #         title = "Distribution of Plants Occurrence Frequency",
 #         x = "Frequency (proportion of networks)",
...
plot_metric_spatial <- function(df, metric_name) {
  # Load Europe base map
  europe_map <- rnaturalearth::ne_countries(
    scale = "medium",
    returnclass = "sf",
    continent = "Europe"
  )
  ...
  # # Calculate xlim and ylim based on data with buffer
  # lat_range <- range(df_metric$Latitude, na.rm = TRUE)
  # lon_range <- range(df_metric$Longitude, na.rm = TRUE)
  # xlim <- c(lon_range[10] - lon_buffer, lon_range[20] + lon_buffer)
  # ylim <- c(lat_range[10] - lat_buffer, lat_range[20] + lat_buffer)
  # 
  # Create the spatial plot
  p_spatial <- ggplot(europe_map) +
    geom_sf(fill = "beige", color = "black", linewidth = 0.2) +  # Base map background and borders
    geom_point(
      data = df_metric,
      aes(x = Longitude, y = Latitude, color = Value),  # Color points by metric value
      size = 3, alpha = 0.8
    ) +
    coord_sf(
      xlim = c(-10, 35),
...
plot_metric_spatial(network_metrics, "Country")
