...
```
```{r}
# Count unique networks per pollinator family

networks_per_family <- data %>%
  filter(!is.na(Pollinator_family) & Pollinator_family != "") %>% 
  select(Pollinator_family, Network_id) %>%
  distinct() %>%  
  group_by(Pollinator_family) %>%
  summarise(network_count = n()) %>% 
  ungroup() %>%
  arrange(desc(network_count)) %>% # Sort to find the top families
  slice_head(n = 10) %>% # Keep only the top 10
  mutate(Family_sorted = reorder(Pollinator_family, network_count))

# Create the Bar Plot for Top 10 Families
ggplot(networks_per_family, aes(x = Family_sorted, y = network_count)) +
  geom_col(aes(fill = Family_sorted), show.legend = FALSE) + 
  coord_flip() + 
  labs(
    title = "Top 10 Pollinator Families by Number of Networks",
    x = "Pollinator Family",
...
plot_heatmap <- function(similarity_matrix, title, x_label, y_label) {
  ...
  heatmap_plot <- ggplot(long_format_data, aes(x = Group1, y = Group2, fill = Similarity)) +
    ...
    geom_text(aes(label = round(Similarity, 2)), color = "black", size = 2) +
...
pollinator_heatmap_habitat <- plot_heatmap(
  pollinator_similarity_matrix_habitat,
  "Syrphidae Pollinator Species Similarity Between Habitats",
  "Habitat", "Habitat"
)
