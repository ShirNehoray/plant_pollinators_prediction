...
# Ensure you have dplyr, ggplot2, and tidyr installed
library(dplyr)
library(ggplot2)
library(tidyr) # For as.data.frame(as.table)

# --- Step 1: Load Data ---

# Read the interaction data
# The file 'Interaction_data.csv' should be in your working directory.
tryCatch({
  data <- read.csv("Interaction_data.csv", encoding = "latin1")
}, error = function(e) {
  stop("Error: 'Interaction_data.csv' not found. Please make sure the file is in your working directory.")
})
...
# --- Step 3: Aggregate Data by Bioregion (from your code) ---
...
# --- Step 4: Aggregate Data by Habitat (from your code) ---
...
# --- Step 5: Similarity Functions (from your code) ---
...
# --- NEW: Function to create a CROSS-similarity matrix (e.g., Bioregion 1 vs Bioregion 2) ---
...
# --- MOVED: Plotting Function (formerly Step 8) ---
...
# --- Step 6: Create Matrices (from your code) ---
...
# --- Step 7: NEW - Find and Print Most Similar Bioregions ---
...
# Make a copy of the pollinator matrix to modify
...
# Find the maximum similarity value, ignoring NAs
max_pollinator_sim <- max(pollinator_matrix_copy, na.rm = TRUE)

# Find the row/column indices of this max value
max_pollinator_indices <- which(pollinator_matrix_copy == max_pollinator_sim, arr.ind = TRUE)

# Take the first pair (since the matrix is symmetrical, [i, j] and [j, i] will both be max)
if(nrow(max_pollinator_indices) > 0) {
  ...
  # --- NEW DETAILED COMPARISON (POLLINATORS) ---
  ...
  # --- NEW: CROSS-HABITAT HEATMAP (POLLINATORS) ---
...
if(nrow(max_plant_indices) > 0) {
  ...
  cat(sprintf("Highest Plant Species Similarity (Bioregions):\n"))
  cat(sprintf("  Bioregion 1: %s\n", bio1_plant_name))
  cat(sprintf("  Bioregion 2: %s\n", bio2_plant_name))
  cat(sprintf("  Jaccard Similarity: %.4f\n", max_plant_sim))
  
  # --- NEW DETAILED COMPARISON (PLANTS) ---
  cat(sprintf("\n  Comparison of these two bioregions (Plants):\n"))
  ...
  # Print the comparison details
  cat(sprintf("    Total unique Plant species in %s: %d\n", bio1_plant_name, length(species_list_1)))
  cat(sprintf("    Total unique Plant species in %s: %d\n", bio2_plant_name, length(species_list_2)))
  cat(sprintf("    Shared Plant species: %d\n", shared_count))
  cat(sprintf("    Total unique Plant species (Union): %d\n", total_count))
  ...
  if(shared_count > 0) {
    ...
    cat("    No shared Plant species found between these two regions.\n\n")
  }
  
  # --- NEW: CROSS-HABITAT HEATMAP (PLANTS) ---
  cat(sprintf("  Generating Habitat Cross-Similarity Heatmap for %s vs %s (Plants)...\n", bio1_plant_name, bio2_plant_name))
  ...
  if (nrow(data_group1_plant) > 0 && nrow(data_group2_plant) > 0) {
    ...
    plant_cross_heatmap <- plot_heatmap(
      ...
      paste("Plant Habitat Similarity:", bio1_plant_name, "vs.", bio2_plant_name),
...
# --- Step 7.5: NEW - Create Presence/Absence Heatmaps for Most Similar Bioregions ---
...
if(nrow(max_pollinator_indices) > 0) {
  ...
  pa_data_poll <- data %>%
    filter(Bioregion %in% c(bio1_poll_name, bio2_poll_name), 
...
# --- For the Plant Pair ---
if(nrow(max_plant_indices) > 0) {
  cat(sprintf("  Generating Presence/Absence plot for: %s vs %s (Plants)\n", bio1_plant_name, bio2_plant_name))
  ...
  if(nrow(pa_data_plant) > 0) {
    pa_heatmap_plants <- ggplot(pa_data_plant, aes(x = Bioregion, y = Plant_accepted_name, fill = as.factor(Presence))) +
      ...
      labs(
        title = paste("Plant Species Presence:", bio1_plant_name, "vs.", bio2_plant_name),
        x = "Bioregion",
        y = "Plant Species"
      ) +
      theme_minimal() +
    ...
    cat("    No Plant data found for one or both bioregions to create Presence/Absence plot.\n")
  }
}
