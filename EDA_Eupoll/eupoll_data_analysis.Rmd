...
# Aggregate coordinates by Network_id and country  
...
# Create the spatial plot with bioregion colors
ggplot(data = europe_map) +
  ...
  theme_minimal() +
  labs(
    title = "Geographical location of all networks in the EuPPollNet",
    x = "Longitude", 
    y = "Latitude",
    subtitle = NULL
  ) +
...
```
```{r}
# Filter coordinates for Estonia only 
...
# Differ according to Study_id

# Filter coordinates for Estonia only 
network_coords_estonia_2 <- interactions %>%
  filter(Country == "Estonia") %>%
  group_by(Network_id) %>%
  summarise(
    Latitude = mean(Latitude, na.rm = TRUE),
  ...
  left_join(network_summary %>% select(Network_id, Study_id), by = "Network_id")

# Get a European map 
europe_map <- ne_countries(scale = "medium", continent = "Europe", returnclass = "sf")

# Zoomed view on Estonia colored by Study
ggplot(data = europe_map) +
  geom_sf(fill = "beige", color = "black", linewidth = 0.2) +
  geom_point(data = network_coords_estonia_2, 
             aes(x = Longitude, y = Latitude, color = Study_id), 
             size = 3, alpha = 0.8) +
  # Use automatic color palette for studies (or manual if you want specific colors)
  scale_color_viridis_d(name = "Study") +
  coord_sf(xlim = c(23, 30), ylim = c(57.5, 60)) +
  theme_minimal() +
  labs(
    title = "Plant-Pollinator Networks in Estonia",
