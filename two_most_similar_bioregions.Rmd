...
if(nrow(max_pollinator_indices) > 0) {
  ...
  if (nrow(pa_data_poll) > 0) {
    pa_heatmap_pollinators <- ggplot(pa_data_poll, aes(x = Bioregion, y = Pollinator_accepted_name, fill = as.factor(Presence))) +
      ...
      labs(
        title = paste("Apidae Species Presence"),
        x = "Bioregion",
        y = "Pollinator Species"
      ) +
      theme_minimal() +
      theme(
        axis.text.x = element_text(angle = 360, vjust = 1, hjust = 1, size = 12),
...
if(nrow(max_plant_indices) > 0) {
  ...
  if(nrow(pa_data_plant) > 0) {
    pa_heatmap_plants <- ggplot(pa_data_plant, aes(x = Bioregion, y = Plant_accepted_name, fill = as.factor(Presence))) +
      ...
      labs(
        title = paste("Plant (Apidae-Interacting) Species Presence"),
        x = "Bioregion",
        y = "Plant Species"
      ) +
      theme_minimal() +
      theme(
        axis.text.x = element_text(angle = 360, vjust = 1, hjust = 1, size = 16),
...
# --- Step 8: Generate and Print Plots ---
...
# --- Generate and print the HABITAT heatmaps ---
...
# --- Print the cross-similarity plots ---
